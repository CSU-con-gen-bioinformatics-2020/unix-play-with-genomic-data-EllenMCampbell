...
```
drwxr-xr-x  18 molecol  staff     612 Jan 30 12:38 .
drwxr-xr-x   7 molecol  staff     238 Jan 29 10:02 ..
-rw-r--r--@  1 molecol  staff    6148 Jan 29 11:47 .DS_Store
-rw-r--r--   1 molecol  staff      39 Jan 30 10:34 .Rhistory
drwxr-xr-x   4 molecol  staff     136 Jan 29 10:02 .Rproj.user
drwxr-xr-x  13 molecol  staff     442 Jan 30 12:42 .git
-rw-r--r--   1 molecol  staff     126 Jan 29 10:02 .gitignore
```
...
```{bash}
mkdir fastq/lonely-fastqs
mkdir fastq/lonely-fastqs/R1
mkdir fastq/lonely-fastqs/R2
```
...
```
So you don't delete everything
```

# If you had just typed `rm -r bad directory name with spaces` what would Unix have tried to remove?

```
a directory called "bad", another called "name", another called "with", and another called "spaces"
```
...
```{bash}
mkdir A05_S5
cp {sam,bam,fastq}/*A05_S5* A05_S5
ls A05_S5
```
...
```{bash}
#Is there a way to do this in fewer lines?
...
```
They end with a "/1" for read 1 and "/2" for read 2
```
...
```{bash}
#Not sure I fully understand what two different things I'm supposed to be doing here. Don't think these two below should be the same.
gzcat fastq/*R1* | wc
wc fastq/*R1*
```
...
```{bash}
gzip R1_all_via_cat.fq
shasum R1_all_via_cat.fq.gz copy_of_R1_all_via_cat.fq.gz
```
...
```
unzipped is ~2x as big as zipped?
```
...
```{bash}
#Not sure I'm reading the prompt for this properly, hopefully this is what I was intended to do
du -h bam/* sam/*
```
...
```{bash}
chmod g+w silly/DIRdrwxrwxr-x
chmod o-r silly/FILE-rw-r-----
chmod g+w silly/FILE-rw-rw-r--
chmod u+x,o-r silly/FILE-rwxr-----
chmod ug+x,g+w silly/FILE-rwxrwxr--
```
